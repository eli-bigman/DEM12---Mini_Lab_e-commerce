...
flowchart TD
    ...
    subgraph Orchestration["Orchestration Layer"]
        direction TB
        AF["Apache Airflow"]
        subgraph Pipeline["ecommerce_pipeline DAG (hourly)"]
            direction LR
            SCAN["Scan MinIO\nfor new files"]
            VALID["Validate\nGreat Expectations"]
            CLEAN["Clean &\nStandardise"]
    ...
    subgraph Storage["Structured Storage"]
        ...
        subgraph Schemas["Schemas"]
            STG_DB["staging.*\n7 tables + processed_files"]
            ANA["analytics.*\nStar Schema"]
            subgraph StarSchema["Star Schema"]
                direction LR
                DIMDATE["dim_date"]
                DIMCUST["dim_customers\n(SCD Type 2)"]
                DIMPROD["dim_products\n(SCD Type 2)"]
                DIMINV["dim_inventory"]
                FORD["fact_orders\n(monthly partitioned)"]
                FPAY["fact_payments"]
                FRET["fact_returns"]
                AGRV["agg_revenue"]
    ...
    subgraph BI["Business Intelligence"]
        direction TB
        MB["Metabase"]
        subgraph Dashboard["Executive Dashboard"]
            KPI["KPI Cards\nRevenue · Orders · AOV · Profit"]
            CHARTS["Charts\nTrend · Category · Channel\nTop Products · Retention · Status"]
        end
        MB --> Dashboard
    end
    ...
    COL -- "CSV micro-batches\nevery 60s" --> MINIO
    VALID -- "invalid rows" --> DLQ
    Pipeline -- "reads new files" --> RAW
    STG -- "UPSERT" --> STG_DB
    XFORM -- "SCD2 / CDC / UPSERT" --> ANA
    MB -- "queries analytics.*\nvia metabase_user" --> PG
    GHA -- "triggers on push/PR" --> Pipeline
```
