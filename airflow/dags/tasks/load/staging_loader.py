...
from utils.db_helper import execute_values
...
def load_customers(df: pd.DataFrame, source_file: str) -> int:
	...
	rows = [
		(
			row.customer_id,
			_safe(row.signup_date),
			_safe(row.country),
			_safe(row.acquisition_channel),
			_safe(row.customer_segment),
			source_file,
		)
...
def load_products(df: pd.DataFrame, source_file: str) -> int:
	...
	rows = [
		(
			...
			_safe(row.category),
			_safe(row.cost_price),
			_safe(row.selling_price),
			_safe(row.inventory_quantity),
			source_file,
		)
		for row in df.itertuples(index=False)
	]
...
def load_orders(df: pd.DataFrame, source_file: str) -> int:
	...
	rows = [
		(
			row.order_id,
			_safe(row.customer_id),
			_safe(row.product_id),
			_safe(row.quantity),
			_safe(row.unit_price),
			_safe(row.total_amount),
			_safe(row.order_timestamp),
			_safe(row.payment_method),
			_safe(row.order_status),
			source_file,
		)
		for row in df.itertuples(index=False)
	]
...
def load_inventory(df: pd.DataFrame, source_file: str) -> int:
	...
	rows = [
		(
			row.inventory_id,
			_safe(row.product_id),
			_safe(row.warehouse_location),
			_safe(row.quantity_on_hand),
			_safe(row.last_restock_date),
			source_file,
		)
		for row in df.itertuples(index=False)
	]
...
def load_revenue(df: pd.DataFrame, source_file: str) -> int:
	...
	rows = [
		(
			row.revenue_id,
			_safe(row.record_date),
			_safe(row.total_revenue),
			_safe(row.total_profit),
			source_file,
		)
		for row in df.itertuples(index=False)
	]
...
def load_returns(df: pd.DataFrame, source_file: str) -> int:
	...
	rows = [
		(
			row.return_id,
			_safe(row.order_id),
			_safe(row.return_date),
			_safe(row.return_reason),
			_safe(row.refund_amount),
			_safe(row.return_status),
			source_file,
		)
		for row in df.itertuples(index=False)
	]
